...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//에디터 내용 저장
		else if(command.equals("/board/editorInsert.do")) {
			String content = request.getParameter("ir1");
			BoardDao bd = new BoardDao();
			int value = bd.editInsert(content);
			System.out.println(value);
			
			response.sendRedirect(request.getContextPath()+"/button.jsp");
		}
		//수정
		else if(command.equals("/board/editorModify.do")) {
			String bidx = request.getParameter("bidx");
			BoardDao bd = new BoardDao();
			BoardVo bv = bd.view(Integer.parseInt(bidx));
			
			request.setAttribute("bv", bv);
			
			RequestDispatcher rd = request.getRequestDispatcher("/editor2.jsp");
			rd.forward(request, response);
		}
		else if(command.equals("/board/editorModifyAction.do")) {
			String bidx = request.getParameter("bidx");
			String content = request.getParameter("ir1");
			BoardDao bd = new BoardDao();
			int value = bd.editUpdate(Integer.parseInt(bidx), content);
			System.out.println(value);
		}
		//삭제
		else if(command.equals("/board/editorDelete.do")) {
			
		}
		//보기
		else if(command.equals("/board/editorView.do")) {
			String bidx = request.getParameter("bidx");
			BoardDao bd = new BoardDao();
			BoardVo bv = bd.view(Integer.parseInt(bidx));
