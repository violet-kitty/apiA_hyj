...
public class BoardDao {
	...
	public int editInsert(String content) {
		int value = 0;
		String sql = "INSERT INTO edit(bidx,content) VALUES(edit_seq.nextval,?)";
		try {
			pstmt = conn.prepareStatement(sql);
	...
	public int editUpdate(int bidx, String content) {
		int value = 0;
	...
	public BoardVo view(int bidx) {
		...
		String sql = "select * from edit where bidx=?";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bidx);
			rs = pstmt.executeQuery();
